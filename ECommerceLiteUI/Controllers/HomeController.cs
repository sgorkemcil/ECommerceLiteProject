    ...
    public class HomeController :BaseController
    {
        ...
        public async Task<ActionResult> Buy()
        {
            try
            {
                ...
                //Artık alışveriş tammalansın.
                Order customerOrder = new Order()
                {
                    CustomerTCNumber = customer.TCNumber,
                    IsDeleted = false,
                    OrderNumber = customer.TCNumber//burayı duzeltecegz..
                };
                ...
                if (orderInsertResult > 0)
                {
                    ...
                    foreach (var item in shoppingcart)
                    {
                        ...
                        orderDetailsInsertResult += myOrderDetailRepo.Insert
                            (customerOrderDetail);
                    }
                    ...
                    {
                        //Sistem yöneticisine orderId detayı verilerek email gönderilsin.Eklenmeyen ürünleri acilen eklesinler
                        var message = $"Merhaba Admin,<br/>" + $"Aşağıdaki bilgileri verilen siparişin kenidisi oluşturulmasına rağmen detaylarından bazıları oluşturulmadı.Acilen müdahale edelim.<br/><br/>" +
                                        $"OrderId:{customerOrder.Id}<br/>" +
                                        $"Sipariş detayları <br/>";
                        for (int i = 0; i <= shoppingcart.Count; i++)
                        {
                            message += $"{i}-{shoppingcart[i].Id}-" +
